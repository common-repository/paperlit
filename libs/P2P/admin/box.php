...
class P2P_Box {
	...
	public function ajax_create_post() {
		...
		$from = absint(intval(esc_attr($_POST['from'])));
	...
	public function ajax_connect() {
		$this->safe_connect(intval(esc_attr($_POST['to'])));
	}
	...
	public function ajax_disconnect() {
		p2p_delete_connection(esc_attr($_POST['p2p_id']) );

		$this->refresh_candidates();
	}

	public function ajax_clear_connections() {
		$r = $this->ctype->disconnect( esc_attr($_POST['from']), 'any' );
	...
	private function refresh_candidates() {
		die( json_encode( $this->candidate_rows(
			esc_attr($_REQUEST['from']), esc_attr($_REQUEST['paged']), esc_attr($_REQUEST['s']) ) ) );
	}
