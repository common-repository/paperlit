jQuery(function($) {
  // color picker init
  $('.paperlit-color-picker').wpColorPicker();
  if ($(".chosen-select").length) {
    $(".chosen-select").chosen();
  }
  // calendar init
  $('#_paperlit_date').datepicker({dateFormat : 'yy-mm-dd'});

  //repeater fields
  var prefix = $('#_paperlit_prefix_bundle_id');
  var subscription = $('#_paperlit_subscription_prefix');

  $( '#_paperlit_prefix_bundle_id').keyup(function() {
    $( '#_paperlit_single_edition_prefix, #_paperlit_subscription_free_prefix, #_paperlit_subscription_prefix, .paperlit_repeater input[type="text"]' ).trigger( "keyup" );
  });

  $('#_paperlit_single_edition_prefix, #_paperlit_subscription_prefix').keyup(function() {
    var autocompleted = $(this).next();
    autocompleted.html(prefix.val() + '.' + $(this).val() );
    $( '.paperlit_repeater input[type="text"], #_paperlit_subscription_free_prefix' ).trigger( "keyup" );
  });

  $('#_paperlit_subscription_free_prefix').keyup(function() {
    var autocompleted = $(this).next();
    autocompleted.html(prefix.val() + '.' + subscription.val() + '.' + $(this).val() );
  });

  $( ".form-table" ).delegate('.paperlit_repeater input[type="text"]','keyup',function() {
    var autocompleted = $(this).parent().find('.repeater-completer');
    autocompleted.html(prefix.val() + '.' + subscription.val() + '.' + $(this).val() );
  });

  $( "#_paperlit_prefix_bundle_id" ).trigger( "keyup" );

  //add cloned field
  $( "#add-field" ).click(function(e) {
    e.preventDefault();

    var clone = $( "#paperlit_repeater" ).clone();
  ...
  $('#paperlit_metabox').removeClass('postbox');
  $('.tabbed').css('display','none');
  var className = $('.taxonomy-paperlit_editorial_project .tabbed').first().attr('class');
  if (className) {
  ...
  $('.nav-tab').click(function(e) {
    ...
    if (tab == 'pad_meta') {
      $( 'input[name="_paperlit_pad_sgs_shelf_backgroundFillStyle"]' ).change();
    } else if (tab == 'phone_meta') {
      $( 'input[name="_paperlit_phone_sgs_shelf_backgroundFillStyle"]' ).change();
    }
  });

  if ($(this).find( 'input[name="_paperlit_pad_sgs_shelf_backgroundFillStyle"]' )) {
    $( 'input[name="_paperlit_pad_sgs_shelf_backgroundFillStyle"]' ).change(function() {
      if ($(this).is(':checked')) {
        var bi = $('label[for="_paperlit_pad_sgs_shelf_backgroundImage"]').closest('tr'),
        bc = $('label[for="_paperlit_pad_sgs_shelf_backgroundFillStyleColor"]').closest('tr'),
        gs = $('label[for="_paperlit_pad_sgs_shelf_backgroundFillGradientStart"]').closest('tr'),
        ge = $('label[for="_paperlit_pad_sgs_shelf_backgroundFillGradientStop"]').closest('tr');
        switch ($(this).val()) {
          default:
            bi.fadeIn();
            bc.hide();
  ...
  if ($(this).find( 'input[name="_paperlit_phone_sgs_shelf_backgroundFillStyle"]' )) {
    $( 'input[name="_paperlit_phone_sgs_shelf_backgroundFillStyle"]' ).change(function() {
      if ($(this).is(':checked')) {
        var bi = $('label[for="_paperlit_phone_sgs_shelf_backgroundImage"]').closest('tr'),
        bc = $('label[for="_paperlit_phone_sgs_shelf_backgroundFillStyleColor"]').closest('tr'),
        gs = $('label[for="_paperlit_phone_sgs_shelf_backgroundFillGradientStart"]').closest('tr'),
        ge = $('label[for="_paperlit_phone_sgs_shelf_backgroundFillGradientStop"]').closest('tr');
        switch ($(this).val()) {
          default:
            bi.fadeIn();
            bc.hide();
  ...
  $('#test-connection').click(function(e) {
    ...
    var server    = $('input[name="_paperlit_ftp_server[0]"]').val();
    var port      = $('input[name="_paperlit_ftp_server[1]"]').val();
    var base      = $('input[name="_paperlit_ftp_destination_path"]').val();
    var user      = $('input[name="_paperlit_ftp_user"]').val();
    var password  = $('input[name="_paperlit_ftp_password"]').val();
    var protocol  = $('input[name="_paperlit_ftp_protocol"]:checked').val();
  ...
  var override_web = $('#_paperlit_web_override_eproject');
  var override_hpub = $('#_paperlit_hpub_override_eproject');
  var override_adps = $('#_paperlit_adps_override_eproject');
  ...
  checkTrasferProtocol( $('input[name="_paperlit_ftp_protocol"]:checked') ); //on document ready

  $('input[name="_paperlit_ftp_protocol"]').live("change",function(e) {
  ...
  function checkTrasferProtocol(element) {
    ...
    switch( value ) {
      ...
      case 'sftp':
        ...
        $('.web_metabox input[name="_paperlit_local_path"]').attr('disabled','disabled');
        $('#test-connection').removeAttr('disabled');
        break;
      case 'local':
        $('.web_metabox input[type="text"], .web_metabox input[type="password"]').attr('disabled','disabled');
        $('.web_metabox input[name="_paperlit_local_path"]').removeAttr('disabled');
