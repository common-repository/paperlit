<?php
final class Paperlit_Server_Feed extends Paperlit_Server_API
{
  ...
  public function add_endpoint() {
    ...
    parent::add_endpoint();
    add_rewrite_rule( '^paperlit-api/newsstand-issue-feed/([^&]+)/?$',
                      'index.php?__paperlit-api=newsstand_issue_feed&editorial_project=$matches[1]',
                      'top' );
    add_rewrite_rule( '^([^/]*)/paperlit-api/newsstand-issue-feed/([^&]+)/?$',
                      'index.php?__paperlit-api=newsstand_issue_feed&editorial_project=$matches[2]',
                      'top' );
  }
...
<summary>' . $post->post_excerpt . '</summary>';
  ...
  protected function _feed_cover_art_icons( $post_id ) {
    ...
    $post_thumbnail_id = get_post_meta( $post_id, '_paperlit_newsstand_issue_cover', true );
  ...
  protected function _action_newsstand_issue_feed() {
    ...
    $latest_issue = Paperlit_Editorial_Project::get_latest_edition( $eproject );
    if ( !$latest_issue ) {
    ...
    $issues = Paperlit_Editorial_Project::get_all_editions( $eproject );
    if ( !empty( $issues ) ) {
...
$paperlit_server_feed = new Paperlit_Server_Feed;
