 ...
 * @return array
 */
function paperlit_get_edition_posts_id( $edition, $only_enabled = true ) {
 ...
 */
function paperlit_get_edition_posts( $edition, $only_enabled = true ) {

  $linked_posts_id = paperlit_get_edition_posts_id( $edition, $only_enabled );

  if ( !empty( $linked_posts_id) ) {

    $custom_post_types = paperlit_get_option( 'paperlit_custom_post_type' );
    if ( is_array( $custom_post_types ) ) {
      $post_types = array_merge( array( 'post', 'page' ), paperlit_get_option( 'paperlit_custom_post_type' ) );
    }
    else {
 ...
 * @return array
 */
function paperlit_get_connected_edition_ids( $post ) {
  ...
  $linked_posts = $wpdb->get_col( $wpdb->prepare( $q, 'trash', PAPERLIT_P2P_EDITION_CONNECTION, $post->ID ) );

  return $linked_posts;
}

/**
 * Get single option from paperlit option array
 *
 * @param  string $option
 * @return mixed or boolean false
 */
function paperlit_get_option( $option ) {

   $configs = get_option( 'paperlit_settings' );
 ...
 * @return mixed or boolean false
 */
function paperlit_get_eproject_option( $term_id, $option ) {

  $option = Paperlit_Editorial_Project::get_config( $term_id, $option);
 ...
 * @return string
 */
function paperlit_book( $edition_id ) {

  $edition = get_post( $edition_id );
  $book_url = str_replace( array( 'http://', 'https://' ), 'book://', PAPERLIT_HPUB_URI );

  return $book_url . Paperlit_Utils::sanitize_string( $edition->post_title . '.hpub' );
}
 ...
 */
function paperlit_get_edition_post( $post_id, $edition, $position = '' ) {
 ...
 * @return string or boolean
 */
function paperlit_prev( $post_id = false, $edition = false ) {
  if( !$edition && isset( $_GET['edition_id'] ) ) {
    $edition_id = esc_attr($_GET['edition_id']);  // escaping
    $edition_id = intval($edition_id);            // validate
    $edition    = get_post( $edition_id );
  }
  if( !$post_id ) {
    global $post;
  ...
  return paperlit_get_edition_post( $post_id, $edition, 'prev' );
}
 ...
 * @return string or boolean
 */
function paperlit_next( $post_id = false, $edition = false ) {
  if( !$edition && isset( $_GET['edition_id'] ) ) {
    $edition_id = esc_attr($_GET['edition_id']);  // escaping
    $edition_id = intval($edition_id);            // validate
    $edition    = get_post($edition_id);
  ...
  return paperlit_get_edition_post( $post_id, $edition, 'next' );
}
 ...
 */
function paperlit_get_sharing_placeholder( $post_id = false, $term_id ) {
  ...
  $sharing_domain = isset( $options['_paperlit_sharing_domain'] ) ? $options['_paperlit_sharing_domain'] : '' ;
  if( $sharing_domain ) {
 ...
 */
function paperlit_get_sharing_url( $post_id = false, $term_id ) {

  if( !$post_id ) {
    global $post;
    $post_id = $post->ID;
  }
  $sharing_url = get_post_meta( $post_id, '_paperlit_sharing_url', true );

  if( $sharing_url ) {
    return $sharing_url;
  }

  return paperlit_get_sharing_placeholder( $post_id, $term_id );
}
 ...
 * @return array
 */
function paperlit_get_galleries( $post_id ) {
  ...
  if( $matched_galleries ) {
    $matched_galleries = paperlit_cycle_matches( $matched_galleries );
  }

  if( $matched_playlist ) {
    $matched_playlist = paperlit_cycle_matches( $matched_playlist );
  }
 ...
 * @return array
 */
function paperlit_cycle_matches( $matches ) {
  ...
  if( $galleries ) {
    ...
    foreach( $galleries as $key => $gallery ) {
      ...
      foreach( $attachments_id as $k => $attachment_id ) {
        ...
        $book_gallery[$k]['uri'] = PAPERLIT_EDITION_MEDIA . $info['basename'];
...
function paperlit_get_template_directory() {

  return get_option('paperlit_theme_root') . DS . get_option( 'stylesheet' );
}

function paperlit_get_template_directory_uri() {

  return get_option('paperlit_theme_uri') . DS . get_option( 'stylesheet' );
}
