...
class P2P_Box_Factory extends P2P_Factory {
	...
	function save_post( $post_id, $post ) {
		...
		if ( isset( $_POST['p2p_connections'] ) ) {
			...
			$p2p_connections = isset($_POST['p2p_connections'] ) ? 
                          wp_unslash($_POST['p2p_connections']) :
                          array();

      		if ( is_array( $p2p_connections ) )  
      		{
				foreach ($p2p_connections as $p2p_id ) {
					$p2p_id = sanitize_text_field($p2p_id);
		...
		{
			$p2p_order = isset($_POST['p2p_order'] ) ? 
                          wp_unslash($_POST['p2p_order']) :
                          array();

      		if ( is_array( $p2p_order ) )  
      		{
				foreach ( $p2p_order as $key => $list ) {
