<?php
/**
 * Paperlit Editorial Project class.
 * Custom taxonomies for Edition custom post type
 */
class Paperlit_Editorial_Project
{
  public $tax_obj;
  ...
  public function __construct() {
    ...
    if ( is_admin() ) {
      ...
      add_filter( 'manage_edit-' . PAPERLIT_EDITORIAL_PROJECT . '_columns', array( $this, 'editorial_project_columns' ) );
      add_filter( 'manage_' . PAPERLIT_EDITORIAL_PROJECT . '_custom_column', array( $this, 'manage_columns' ), 10, 3 );
      add_filter( 'wp_tag_cloud', array( $this, 'remove_tag_cloud' ), 10, 2 );

      add_action( PAPERLIT_EDITORIAL_PROJECT . '_edit_form', array( $this, 'add_tabs_to_form' ), 10, 2 );
      add_action( PAPERLIT_EDITORIAL_PROJECT . '_edit_form_fields', array( $this, 'edit_form_meta_fields' ), 10, 2 );
      add_action( PAPERLIT_EDITORIAL_PROJECT . '_add_form', array( $this,'customize_form' ) );
      add_action( PAPERLIT_EDITORIAL_PROJECT . '_edit_form', array( $this,'customize_form' ) );
      add_action( 'edited_' . PAPERLIT_EDITORIAL_PROJECT, array( $this, 'update_form_meta_fields' ) );
      add_action( 'create_' . PAPERLIT_EDITORIAL_PROJECT, array( $this, 'save_form_meta_fields' ) );
      add_action( PAPERLIT_EDITORIAL_PROJECT . '_term_edit_form_tag', array( $this,'form_add_enctype' ) );
      ...
      remove_action( 'wp_ajax_add-' . PAPERLIT_EDITORIAL_PROJECT, '_wp_ajax_add_hierarchical_term' );
      add_action( 'wp_ajax_add-' . PAPERLIT_EDITORIAL_PROJECT, array( $this, 'add_non_hierarchical_term' ) );
  ...
  */
  ...
  public function add_editorial_project_taxonomy() {
    ...
    register_taxonomy( PAPERLIT_EDITORIAL_PROJECT, PAPERLIT_EDITION, $args );
    $this->tax_obj = get_taxonomy( PAPERLIT_EDITORIAL_PROJECT );
  }
  ...
  */
  public function manage_columns( $out, $column_name, $editorial_id ) {
    ...
    $editorial = get_term( $editorial_id, PAPERLIT_EDITORIAL_PROJECT );
    switch ( $column_name ) {

       case 'actions':
          //$shelf_url = home_url( 'paperlit-api/shelf/' . $editorial->slug );
          $shelf_url = PAPERLIT_IOS_SETTINGS_URI . $editorial->slug . '.xml';
          $newsstand_url = home_url( 'paperlit-api/newsstand-issue-feed/' . $editorial->slug );
          ...
          echo '<a href="#" data-term="'.$editorial_id.'" class="paperlit-reset" style="color:#A00">Restore default settings</a>';
          break;
       default:
          break;
    }
  ...
  public function get_custom_metabox( $term_id ) {
    ...
    $metaboxes = array();
    do_action_ref_array( 'paperlit_add_eproject_tab', array( &$metaboxes, $term_id ) );
  ...
  public function add_tabs_to_form( $term ) {
    ...
    echo '<div id="paperlit_metabox">';
    echo '<div class="press-header postbox">';
    echo '<div class="press-container">';
    echo '<i class="press-paperlit-logo-gray-wp"></i>';
  ...
  public function reset_editorial_project() {
    $term_id = intval(esc_attr($_POST['term_id']));
  ...
  */
  public function save_form_meta_fields( $term_id ) {
    $terms = get_terms( PAPERLIT_EDITORIAL_PROJECT );
    foreach( $terms as $term ) {
      $option = self::get_config( $term->term_id, '_paperlit_prefix_bundle_id');
      if ( isset($_POST['_paperlit_prefix_bundle_id']) && strlen($_POST['_paperlit_prefix_bundle_id']) && $option == sanitize_text_field($_POST['_paperlit_prefix_bundle_id']) && $term->term_id != $term_id ) {
        $url = admin_url( 'edit-tags.php?action=edit&post_type='. PAPERLIT_EDITION .'&taxonomy=' . PAPERLIT_EDITORIAL_PROJECT . '&tag_ID=' . $term_id . '&pmtype=error&pmcode=duplicate_entry&pmparam=app_id' );
        wp_redirect( $url );
        exit;
      }
    }
    ...
    //   $url = admin_url( 'edit-tags.php?action=edit&post_type='. PAPERLIT_EDITION .'&taxonomy=' . PAPERLIT_EDITORIAL_PROJECT . '&tag_ID=' . $term_id );
    //   wp_redirect( $url );
    //   exit;
    // }
  ...
  ?>
    <script type="text/javascript">
      jQuery(function(){
        ...
        jQuery('#paperlit_metabox').appendTo('#moved-tab');
        jQuery('.submit').appendTo('.press-header-right');
        jQuery('input[name="paperlit_push_service"]').change(function(){
          var $this = jQuery(this);
          if ( $this.is(':checked') ) {
            if ( $this.val() == 'urbanairship' ) {
              jQuery('label[for="paperlit_push_api_app_id"]').html('App Key');
              jQuery('label[for="paperlit_push_api_key"]').html('App Master Secret');
              jQuery('#paperlit_push_api_app_id').next('.description').html('Urban Airship generated string identifying the app setup. Used in the application bundle.');
              jQuery('#paperlit_push_api_key').next('.description').html('Urban Airship generated string used for server to server API access. This should never be shared or placed in an application bundle.');
            } else if ( $this.val() == 'parse' ) {
              jQuery('label[for="paperlit_push_api_app_id"]').html('Application ID');
              jQuery('label[for="paperlit_push_api_key"]').html('REST API Key');
              jQuery('#paperlit_push_api_app_id').next('.description').html('This is the main identifier that uniquely specifies your application. This is paired with a key to provide your clients access to your application\'s data.');
              jQuery('#paperlit_push_api_key').next('.description').html('This key should be used when making requests to the REST API. It also adheres to object level permissions.');
            }
          }
        }).change();
      });
  ...
  public function remove_tag_cloud ( $return, $args ) {
    if ( $args['taxonomy'] == PAPERLIT_EDITORIAL_PROJECT ) {
      return false;
    }
  }
  ...
  public static function get_by_slug( $slug ) {
    ...
    $eproject = get_term_by( 'slug', $slug, PAPERLIT_EDITORIAL_PROJECT );
    return $eproject;
  }
 ...
 */
  public static function get_subscription_method( $editorial_project_id, $product_id ) {
    ...
    $subscription_methods = isset( $options['_paperlit_subscription_method'] ) ? $options['_paperlit_subscription_method'] : [];
  ...
  public static function get_subscriptions_id( $editorial_project_id ) {
    ...
    $subscription_types = isset( $options['_paperlit_subscription_types'] ) ? $options['_paperlit_subscription_types'] : [];
    if ( !empty( $subscription_types ) ) {
      foreach ( $subscription_types as $type ) {
        $identifier = $options['_paperlit_prefix_bundle_id'] . '.' . $options['_paperlit_subscription_prefix']. '.' . $type;
  ...
  /**
	...
	public static function get_free_subscription_id( $editorial_project_id ) {
		...
		$options = self::get_configs( $editorial_project_id );
		if ( $options && isset( $options['_paperlit_prefix_bundle_id'], $options['_paperlit_subscription_free_prefix'] ) ) {
      $free_subscription_id = $options['_paperlit_prefix_bundle_id'] . '.' . $options['_paperlit_subscription_prefix']. '.' . $options['_paperlit_subscription_free_prefix'];
		}
  ...
  public static function get_editions_in_range( $eproject, $start_date, $end_date ) {
    ...
    $editions_query = new WP_Query( array(
      ...
      'meta_key'        => '_paperlit_date',
      'orderby'         => 'meta_value',
      'tax_query'       => array(
        array(
          'taxonomy'  => PAPERLIT_EDITORIAL_PROJECT,
          'field'     => 'slug',
          'terms'     => $eproject->slug
        )
      ),
  ...
  public static function get_bundle_id( $editorial_project_id ) {
    ...
    if ( $eproject_options ) {
      $eproject_bundle_id = isset( $eproject_options['_paperlit_prefix_bundle_id'] ) ? $eproject_options['_paperlit_prefix_bundle_id'] : '';
    }
  ...
  public function remove_upload_file_callback() {
    $editorial_project_id = intval(esc_attr($_POST['term_id']));
  ...
  public function add_custom_scripts() {
    global $pagenow;
    if( $pagenow == 'edit-tags.php' && sanitize_text_field($_GET['taxonomy']) == PAPERLIT_EDITORIAL_PROJECT ) {
      wp_register_script( 'eproject', PAPERLIT_ASSETS_URI . '/js/paperlit.eproject.min.js', array( 'jquery' ), '1.0', true );
  ...
  public function remove_meta_box() {
    ...
    if( !is_wp_error( $this->tax_obj ) && isset( $this->tax_obj->object_type ) ) {
      foreach ( $this->tax_obj->object_type as $post_type ) {
        $id = !is_taxonomy_hierarchical( PAPERLIT_EDITORIAL_PROJECT ) ? 'tagsdiv-'. PAPERLIT_EDITORIAL_PROJECT : PAPERLIT_EDITORIAL_PROJECT .'div' ;
  ...
  public function add_meta_box() {
    ...
    if( ! is_wp_error( $this->tax_obj ) && isset( $this->tax_obj->object_type ) ) {
      foreach ( $this->tax_obj->object_type as $post_type ) {
        ...
        $id = !is_taxonomy_hierarchical( PAPERLIT_EDITORIAL_PROJECT ) ? 'radio-tagsdiv-' . PAPERLIT_EDITORIAL_PROJECT : 'radio-' . PAPERLIT_EDITORIAL_PROJECT . 'div' ;
        add_meta_box( $id, $label ,array( $this,'metabox' ), $post_type , 'side', 'core', array( 'taxonomy'=> PAPERLIT_EDITORIAL_PROJECT ) );
      }
    }
  }
  ...
  */
  public function filter_terms_checklist_args( $args ) {
    ...
    if( isset( $args['taxonomy']) && PAPERLIT_EDITORIAL_PROJECT == $args['taxonomy'] ) {
  ...
  */
  public function add_non_hierarchical_term(){
    ...
    wp_die( -1 );
    $names = explode(',', sanitize_text_field($_POST['new'.$taxonomy->name]));
  ...
  */
  function save_single_term( $post_id ) {
    ...
    if ( is_array( PAPERLIT_EDITION ) && isset( $_REQUEST['post_type'] ) && ! in_array ( sanitize_text_field($_REQUEST['post_type']), PAPERLIT_EDITION ) )
    return $post_id;

    // verify nonce
    if ( isset( $_POST["_radio_nonce-{PAPERLIT_EDITORIAL_PROJECT}"]) && ! wp_verify_nonce( sanitize_text_field($_REQUEST["_radio_nonce-{PAPERLIT_EDITORIAL_PROJECT}"]), "radio_nonce-{PAPERLIT_EDITORIAL_PROJECT}" ) )
    return $post_id;

    // OK, we must be authenticated by now: we need to find and save the data

    if ( isset( $_REQUEST["radio_tax_input"][PAPERLIT_EDITORIAL_PROJECT] ) ) {
      $terms_date = isset( $_REQUEST['radio_tax_input'][PAPERLIT_EDITORIAL_PROJECT] ) ? 
                          wp_unslash( $_REQUEST['radio_tax_input'][PAPERLIT_EDITORIAL_PROJECT] ) :
                          array();
      $terms = array();

      if ( is_array( $terms_date ) )  {
        foreach ( $terms_date as $taxonomy => $taxonomy_terms ) {
          $terms[] = sanitize_text_field( $taxonomy_terms );
        }
      }
      ...
      if ( current_user_can( $this->tax_obj->cap->assign_terms ) ) {
        wp_set_object_terms( $post_id, $single_term, PAPERLIT_EDITORIAL_PROJECT );
      }
    }
...
$paperlit_editorial_project = new Paperlit_Editorial_Project();
