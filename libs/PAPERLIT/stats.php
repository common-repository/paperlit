<?php
/**
* Paperlit_Statistics class.
*/

class Paperlit_Stats
{
  ...
  public function hooks() {
    ...
    if ( is_admin() ) {
      ...
      add_action( 'wp_dashboard_setup', array( $this, 'add_paperlit_stats_db_widgets' ) );
    }
  }
  ...
  */
  public function register_dashboard_scripts() {
    ...
    if( $pagenow == 'index.php' ) {
      wp_register_script( 'chartjs', PAPERLIT_ASSETS_URI . '/js/chartjs.min.js', array( 'jquery'), '1.0', true );
   ...
   * @void
   */
  public function add_paperlit_stats_db_widgets() {
  ...
  public static function increment_counter( $scenario, $object_id = 0, $count = 1 ) {
    ...
    $counter = $wpdb->get_var( $wpdb->prepare( "SELECT counter FROM " . $wpdb->prefix . PAPERLIT_TABLE_STATS . " WHERE scenario = %s AND object_id = %d AND stat_date = %d", $scenario, $object_id, $date ), 0, 0 );
    if ( is_null( $counter ) ) {
      $wpdb->insert( $wpdb->prefix . PAPERLIT_TABLE_STATS, array(
        'scenario'    => $scenario,
        'object_id'   => $object_id,
        'stat_date'   => $date,
        'counter'     => 1
      ...
      $wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . PAPERLIT_TABLE_STATS . " SET counter = counter + 1 WHERE scenario = %s AND object_id = %d AND stat_date = %d", $scenario, $object_id, $date ) );
    }
  }
  ...
  public static function get_counter( $scenario, $object_id = 0 ) {
    ...
    global $wpdb;
    $counter = $wpdb->get_var( $wpdb->prepare( "SELECT SUM(counter) AS counter FROM " . $wpdb->prefix . PAPERLIT_TABLE_STATS . " WHERE scenario = %s AND object_id = %d GROUP BY object_id", $scenario, $object_id ), 0, 0 );
...
$paperlit_stats = new Paperlit_Stats();
