...
/**
 * Paperlit_List_Table class.
 *
 * @extends WP_List_Table
 */
class Paperlit_List_Table extends WP_List_Table
{
  ...
  */
  public function prepare_items() {
    ...
    $per_page = !empty( $_REQUEST['post_per_page'] ) ?  intval(esc_attr($_REQUEST['post_per_page'])): 10;
    ...
    $this->set_pagination_args( array(
       ...
       'orderby'      => !empty( $_REQUEST['orderby'] ) && strlen( $_REQUEST['orderby'] ) ? sanitize_text_field($_REQUEST['orderby']) : 'title',
       'order'        => !empty( $_REQUEST['order'] ) && strlen( $_REQUEST['order'] ) ? sanitize_text_field($_REQUEST['order']) : 'asc',
    ) );
  }
  ...
  public function usort_reorder( $a, $b ) {
   ...
   public function column_template( $item ) {
    ...
    $current_theme = get_post_meta( $this->_edition_id, '_paperlit_theme_select', true );
    ...
    if ( $current_theme ) {
      if ( array_key_exists( $current_theme, $themes ) ) {
        if ( $themes[$current_theme]['active'] ) {
          ...
          if( has_action( "paperlit_presslist_{$item->post_type}" ) ) {
            do_action_ref_array( "paperlit_presslist_{$item->post_type}", array( $item, &$html ) );
          }
    ...
    */
	public function ajax_update_post_order() {
      ...
      if (sanitize_text_field($_POST['event']) == 'sort-posts' ) {
         $posts = explode(',', sanitize_text_field($_POST['order']));
         $paged = (int)intval(esc_attr($_POST['currentPage']));
         $post_per_page = (int)intval(esc_attr($_POST['postPerPage']));
   ...
   public function ajax_bulk_callback() {
      ...
      if ( !empty( $_POST['connected_posts'] ) ) {
        $connected_posts = sanitize_text_field($_POST['connected_posts']);
        foreach ($connected_posts as $post_id ) {

          if (sanitize_text_field($_POST['action_to_do']) === 'include' ) {
   ...
   public function ajax_fetch_presslist_callback() {
      ...
      $presslist = new Paperlit_List_Table();
    ...
    $value = p2p_get_meta( intval(esc_attr($_POST['id'])), 'status', true );

    if ( p2p_update_meta( intval(esc_attr($_POST['id'])), 'status', !$value ) ) {
      wp_send_json( $value );
		}
	...
	public function ajax_register_template_callback() {
      ...
      if ( isset( $_POST['template'] ) ) {
        if ( p2p_update_meta(intval(esc_attr($_POST['id'])), 'template', ( esc_attr($_POST['template']) ? esc_attr($_POST['template']) : '' ) ) ) {
           echo 'updated';
        }
      }
      else {
        p2p_update_meta( intval(esc_attr($_POST['id'])), 'template', null );
      }
		exit;
   }
  ...
  public static function add_presslist_scripts() {
    ...
    wp_register_script( 'presslist-ajax', PAPERLIT_ASSETS_URI . 'js/presslist_ajax.js', array( 'jquery' ), '1.0', true );
    wp_register_script( 'presslist-drag-drop', PAPERLIT_ASSETS_URI . 'js/presslist_drag_drop.js', array( 'jquery' ), '1.0', true );
   ...
   protected function _ajax_response_callback() {
      ...
      $total_pages = $this->_pagination_args['total_pages'];
      ob_start();
      if ( !empty( esc_attr($_REQUEST['no_placeholder']) ) )
           $this->display_rows();
      else
  ...
  public function get_linked_posts() {
    ...
    if ( isset( $_GET['edition_id'] ) ) {
      $this->_edition_id = (int)intval(esc_attr($_GET['edition_id']));
    }
    else {
    ...
    $data = array();
    $query_post = Paperlit_Edition::get_linked_posts( $this->_edition_id );
...
$paperlit_list_table = new Paperlit_List_Table();
