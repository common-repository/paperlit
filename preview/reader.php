...
if ( !isset( $_GET['edition_id']) || !strlen( $_GET['edition_id'] ) ) {
  wp_die( __( "<b>Error getting required params. Please check your url address.</b>", 'paperlit' ) );
}

if( !isset( $_GET['package_type']) ) {
  wp_die( __( "<b>Error: missing package type </b>", 'paperlit' ) );
}

$edition_id = esc_attr($_GET['edition_id']);  // escaping
$edition_id = intval($edition_id);            // validate

$package_type = esc_attr($_GET['package_type']);    //escaping
$package_type = sanitize_text_field($package_type); //sanitize

if ( !isset( $_GET['post_id'] ) || !strlen( $_GET['post_id'] ) ) {
  $linked_posts = Paperlit_Preview::init( $edition_id );
  $concurrent_slides = min( count( $linked_posts ), CONCURRENT_PAGES );
} else {
  $post_id = esc_attr($_GET['post_id']);  // escaping
  $post_id = intval($post_id);            // validate
  $linked_posts = array( get_post($post_id) );
  $concurrent_slides = 1;
}

if ( empty( $linked_posts ) ) {
  wp_die( __( "<b>There was an error while trying to build the issue preview.</b><p>Suggestions:</p><ul>
  <li>Check if the issue with id <b>$edition_id</b> exist</li><li>Ensure that there is least one post visible</li></ul>", 'paperlit' ) );
}

$terms = wp_get_post_terms( $edition_id , PAPERLIT_EDITORIAL_PROJECT );

if ( empty( $terms ) ) {
  wp_die( __( "<b>There was an error while trying to build the issue preview.</b><p>Suggestions:</p><ul>
  <li>Ensure that there is least one editorial project linked to this edition</li></ul>", 'paperlit' ) );
}

$edition = get_post( $edition_id );
$edition_name = Paperlit_Utils::sanitize_string( $edition->post_title );
$index_height = get_post_meta( $edition->ID, '_paperlit_index_height', true );
...
<head>
  ...
  wp_head();
  ?>
  <link rel="stylesheet" href=<?php echo PAPERLIT_ASSETS_URI ."/css/preview/preview.ish.min.css" ?>>
  <link rel="stylesheet" type="text/css" href=<?php echo PAPERLIT_ASSETS_URI ."/css/preview/preview.min.css" ?> />
  <link rel="stylesheet" type="text/css" href=<?php echo PAPERLIT_ASSETS_URI . "/css/preview/idangerous.swiper.min.css" ?> />
</head>
<body>
<header class="sg-header" role="banner">
  ...
  <div id="sg-nav-container" class="sg-nav-container">
    ...
    <ol class="sg-nav">
      <li>
        <a class="logo" title="Paperlit">Paperlit</a>
      </li>
      <li>
...
<div id="sg-vp-wrap">
  ...
  <div id="sg-gen-container">
    ...
    <div id="toc" <?php echo $style_height ?>>
      <iframe width="100%" height="<?php echo $intheight ?>" frameborder="0" scrolling="no" src="<?php echo PAPERLIT_PREVIEW_URI . $edition_name . DS . "index.html"  ?>"></iframe>
    <?php 
    ...
    ?>
    <div id="toc">
      <iframe width="100%" id="toc-frame" frameborder="0" src="<?php echo PAPERLIT_PREVIEW_URI . $edition_name . DS . "index.html"  ?>"></iframe>
    <?php 
...
<script src="<?php echo PAPERLIT_ASSETS_URI . "/js/preview/idangerous.swiper.min.js"?>"></script>
<script src="<?php echo PAPERLIT_ASSETS_URI . "/js/preview/idangerous.swiper.hashnav.min.js"?>"></script>
<script src="<?php echo PAPERLIT_ASSETS_URI . "/js/preview/paperlit.reader.min.js"?>"></script>
<?php if ($intheight > 0) : ?>
<script src="<?php echo PAPERLIT_ASSETS_URI . "/js/preview/paperlit.toc.js"?>"></script>
<?php else: ?>
<script src="<?php echo PAPERLIT_ASSETS_URI . "/js/preview/paperlit.toc-full.js"?>"></script>
<?php endif; ?>
<script src="<?php echo PAPERLIT_ASSETS_URI . "/js/preview/paperlit.reader.ish.min.js"?>"></script>
</body>
</html>
<?php exit; ?>