 ...
 */
class Paperlit_Logs_Table extends WP_List_Table
{
  protected $_per_page;
  ...
  */
  public function prepare_items() {
    ...
    $sortable = '';
    $hidden = array();
    $paperlit_logs = new Paperlit_Logs();
    ...
    $data = $paperlit_logs->get_logs( $start_date, $end_date );
  ...
  public function get_columns() {
    ...
    $columns = array(
      'object_id'    => __( 'Issue', 'paperlit_logs' ),
      'log_date'     => __( 'Date', 'paperlit_logs' ),
      'log_author'   => __( 'Author', 'paperlit_logs' ),
      'log_ip'       => __( 'IP', 'paperlit_logs' ),
      'log_action'   => __( 'Action', 'paperlit_logs' ),
      'log_type'     => __( 'Package type', 'paperlit_logs' ),
      'log_detail'   => __( 'Detail', 'paperlit_logs' ),
    );
  ...
  public function usort_reorder( $a, $b ) {
    ...
    public function column_log_author( $item ) {
      ...
      if ( ! empty( $item->author ) && 0 !== (int) $item->author ) {
        ...
        if ( $user instanceof WP_User && 0 !== $user->ID ) {
          //$user->display_name
          return sprintf(
            '<a href="%s">%s <br/><span class="paperlit-author-name">%s</span></a><br /><small>%s</small>',
            get_edit_user_link( $user->ID ),
            get_avatar( $user->ID, 40 ),
            $user->display_name,
            isset( $user->roles[0] ) && isset( $wp_roles->role_names[ $user->roles[0] ] ) ? $wp_roles->role_names[ $user->roles[0] ] : __( 'Unknown', 'paperlit_logs' )
          );
        }
      }
      return sprintf(
        '<span class="paperlit-author-name">%s</span>',
        __( 'Guest', 'paperlit_logs' )
      );
    }
   ...
   public function display_tablenav( $which ) {
     ...
     <input type="text" value="' .$start_date . '"  class="paperlit-date" id="log_start_date" />
     <input type="text" value="' .$end_date . '"  class="paperlit-date" id="log_end_date" />
   ...
   public function ajax_fetch_logslist_callback() {
      ...
      $presslogs = new Paperlit_Logs_Table();
  ...
  public static function add_logslist_scripts() {
    ...
    global $pagenow;
    if( $pagenow == 'admin.php' && $_GET['page'] == 'paperlit-logs' ) {
      wp_register_style( 'log_page', PAPERLIT_ASSETS_URI . 'css/jquery.datetimepicker.min.css' );
      wp_enqueue_style( 'log_page' );
      wp_register_script( 'log_moment', PAPERLIT_ASSETS_URI . '/js/moment.min.js' );
      wp_enqueue_script( 'log_moment' );
      wp_register_script( 'log_moment_tz', PAPERLIT_ASSETS_URI . '/js/moment.timezone.min.js', array( 'log_moment' ) );
      wp_enqueue_script( 'log_moment_tz' );
      wp_register_script( 'log_datepicker', PAPERLIT_ASSETS_URI . '/js/jquery.datetimepicker.min.js', array( 'jquery' ), '1.0', true );
      wp_enqueue_script( 'log_datepicker' );

      wp_register_script( 'logslist-ajax', PAPERLIT_ASSETS_URI . 'js/logslist_ajax.js', array( 'jquery' ), '1.0', true );
  ...
  public function add_custom_styles() {
    ...
    global $pagenow;
    if( $pagenow == 'admin.php' && $_GET['page'] == 'paperlit-logs' ) {
      wp_register_style( 'paperlit_logs', PAPERLIT_ASSETS_URI . 'css/paperlit.logs.css' );
      wp_enqueue_style( 'paperlit_logs' );
    }
  }
}

$paperlit_logs_table = new Paperlit_Logs_Table();
