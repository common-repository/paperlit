...
class Paperlit_Metabox
{
      ...
      switch ( $field['type'] ) {
        ...
        case 'file':
          if ( !empty( $_FILES[$field_id]['name'] ) ) {
            ...
            $file_types = wp_check_filetype( basename( sanitize_text_field($_FILES[$field_id]['name']) ) );
            $uploaded_type = $file_types['type'];
            // Check if the type is supported. If not, throw an error.
            if ( in_array( $uploaded_type, $supported_types ) ) {
              $upload_overrides = array( 'test_form' => false );
              $uploaded = wp_handle_upload( sanitize_text_field($_FILES[$field_id]), $upload_overrides );
              if ( isset( $uploaded['file'] ) ) {
                $file = $uploaded['file'];
                $attachment = array(
                   'post_mime_type'   => $uploaded_type,
                   'post_title'       => sanitize_text_field($_FILES[$field_id]['name']),
                   'post_content'     => '',
                   'post_status'      => 'inherit'
                );
        ...
        case 'date':
          if ( isset( $_POST[$field_id] ) ) {
            $new_value = date('Y-m-d', strtotime(sanitize_text_field($_POST[$field_id])));
          }
          break;
        case 'repeater':
          if ( isset( $_POST[$field_id] ) ) {
            foreach (sanitize_text_field($_POST[$field_id]) as $key => $single ) {
              $new_value[$key] = $single;
            }
          } else {
            $new_value[$key] = sanitize_text_field($_POST[$field_id][0]);
          }
          break;
      }
  ...
  public function save_term_values( $updated = false ) {
    ...
    foreach ( $this->fields as $field ) {
      ...
      switch ( $field['type'] ) {
        ...
        default:
          $term_meta[$field_id] = isset( $_POST[$field_id] ) ? sanitize_text_field($_POST[$field_id]) : $default;
          break;
        case 'color_clear':
          if ( isset( $_POST[$field_id . '_clear'] ) ) {
          ...
          break;
        case 'double_text':
          $term_meta[$field_id][0] = isset( $_POST[$field_id][0] ) ? sanitize_text_field($_POST[$field_id][0]) : $default;
          $term_meta[$field_id][1] = isset( $_POST[$field_id][1] ) ? sanitize_text_field($_POST[$field_id][1]) : $default;
          break;
        case 'repeater':
          $term_meta[$field_id] = array_filter( sanitize_text_field($_POST[$field_id]) );
          break;
        case 'repeater_with_radio':
          ...
          if ( isset( $_POST[$field['radio_field']] ) ) {
            $term_meta[$field['radio_field']] = array_filter( sanitize_text_field($_POST[$field['radio_field']]) );
          }
          break;
        case 'date':
          $term_meta[$field_id] = date( 'Y-m-d', strtotime( sanitize_text_field($_POST[$field_id]) ) );
          break;
        case 'file':
    ...
    foreach( $this->fields as $field ) {
      ...
      switch ( $field['type'] ) {
        ...
        case 'text_autocompleted':
          $html.= '<input type="text" name="' . $field['id'] . '" id="' . $field['id'] . '" value="' . ( $meta_value ? $meta_value : $field['default'] ) . '" size="20" style="width:100%" /><div class="paperlit_autocompleted"></div>';
          break;
        case 'textarea':
        ...
        case 'file':
          ...
          if ( $meta_value ) {
            ...
            if ( $img ) {
              $html.= '<div class="paperlit-image-container">';
              $html.= $img;
        ...
        case 'date':
          ...
          break;
        case 'color':
          $html.= '<input type="text" name="' . $field['id'] . '" id="' . $field['id'] . '" value="'. ( $meta_value ? $meta_value : $field['default'] ) . '" class="paperlit-color-picker" data-default-color="#ffffff" />
          <p class="description">'. $field['desc'] . '</p>';
          break;
        case 'color_clear':
          $html.= '<input type="text" name="' . $field['id'] . '" id="' . $field['id'] . '" value="'. ( $meta_value == 'clear' ? '' : ( $meta_value ? $meta_value : $field['default'] ) ) . '" class="paperlit-color-picker" data-default-color="#ffffff" />
          <label for="' . $field['id'] . '_clear" class="paperlit-picker-clear"><input type="checkbox" name="' . $field['id'] . '_clear" id="' . $field['id'] . '_clear" ' . ( $meta_value == 'clear' ? 'checked="checked"' : '' ) . ' />' . __("Transparent", 'paperlit_metabox') . '</label>
          <p class="description">'. $field['desc'] . '</p>';
          break;
        case 'number':
        ...
        case 'repeater':
          ...
          foreach ( $repetitions as $value ) {
            ...
            $html.= '<div class="paperlit_repeater" id="paperlit_repeater" data-index="'.$i.'">
        ...
        case 'repeater_with_radio':
          ...
          foreach ( $values as $value ) {
            ...
            $html.= '<div class="paperlit_repeater subscription" id="paperlit_repeater" data-index="'. $i .'">
            <a href="#" ' . ( $i == 0 ? "id=\"add-field\" class=\"add-field\"" : "id=\"remove-field\" class=\"remove-field\"" ). '">' . ( $i == 0 ? $img_add : $img_remove ). '</a>
            ...
            foreach( $field['radio_options'] as $k => $option) {
              ...
              $html.= '<input type="radio" id="checkbox-' . $option['value'].'" name="_paperlit_subscription_method['. $i .']" '. ( $checked ? 'checked="checked"' : '' ) .' value="'.$option['value'].'" />
  ...
  public function add_custom_script() {
    ...
    wp_register_script( 'metabox', PAPERLIT_ASSETS_URI . '/js/paperlit.metabox.js', array( 'jquery', 'wp-color-picker' ), '1.0', true );
