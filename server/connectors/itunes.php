...
final class Paperlit_Connector_iTunes extends Paperlit_Server_API {
  ...
  public function __construct() {
    ...
    add_action( 'paperlit_issue_download', array( $this, 'validate_purchases_on_download' ), 10, 6 );
  }
  ...
  public function add_endpoint() {
    ...
    parent::add_endpoint();
    add_rewrite_rule( '^paperlit-api/itunes_purchase_confirmation/([^&]+)/([^&]+)/([^&]+)/?$',
                      'index.php?__paperlit-api=itunes_purchase_confirmation&app_id=$matches[1]&user_id=$matches[2]&editorial_project=$matches[3]',
                      'top' );
    add_rewrite_rule( '^([^/]*)/paperlit-api/itunes_purchase_confirmation/([^&]+)/([^&]+)/([^&]+)/?$',
                      'index.php?__paperlit-api=itunes_purchase_confirmation&app_id=$matches[2]&user_id=$matches[3]&editorial_project=$matches[4]',
                      'top' );
    add_rewrite_rule( '^paperlit-api/itunes_purchases_list/([^&]+)/([^&]+)/([^&]+)/?$',
                      'index.php?__paperlit-api=itunes_purchases_list&app_id=$matches[1]&user_id=$matches[2]&editorial_project=$matches[3]',
                      'top' );
    add_rewrite_rule( '^([^/]*)/paperlit-api/itunes_purchases_list/([^&]+)/([^&]+)/([^&]+)/?$',
                      'index.php?__paperlit-api=itunes_purchases_list&app_id=$matches[2]&user_id=$matches[3]&editorial_project=$matches[4]',
                      'top' );
  }
  ...
  public function get_shared_secret() {
    ...
    return Paperlit_Editorial_Project::get_config( $this->eproject->term_id , '_paperlit_itunes_secret' );
  }
  ...
  public function save_receipt( $receipt, $transaction_id, $purchase_type ) {
    ...
    $sql = "SELECT receipt_id FROM " . $wpdb->prefix . PAPERLIT_TABLE_RECEIPTS . " ";
    ...
    if ( !$receipt_id ) {
      ...
      $sql = "INSERT IGNORE INTO " . $wpdb->prefix . PAPERLIT_TABLE_RECEIPTS . " SET ";
  ...
  public function get_latest_subscription_receipt() {
    ...
    global $wpdb;
    $sql = "SELECT DISTINCT base64_receipt, transaction_id FROM " . $wpdb->prefix . PAPERLIT_TABLE_RECEIPTS;
  ...
  public function get_purchased_editions() {
    ...
    global $wpdb;
    $sql = "SELECT DISTINCT product_id FROM " . $wpdb->prefix . PAPERLIT_TABLE_RECEIPTS ;
  ...
  public function get_editions_in_subscription( $receipt_data ) {
    ...
    $free_subscription_id = Paperlit_Editorial_Project::get_free_subscription_id( $this->eproject->term_id );
    if ( $free_subscription_id && $free_subscription_id == $subscription_bundle_id ) {
      $to_date = $today;
    }

    // Get the editorial project settings
    $eproject_options = Paperlit_Editorial_Project::get_configs( $this->eproject->term_id );
    // Get the last published edition in editorial project and retrieve the date
    $last_edition_date = false;
    $last_edition = Paperlit_Editorial_Project::get_latest_edition( $this->eproject );
    if ( $last_edition ) {
      $last_edition_date = get_post_meta( $last_edition, '_paperlit_date', true );
    }

    $subscription_method = Paperlit_Editorial_Project::get_subscription_method( $this->eproject->term_id, $subscription_bundle_id );
    ...
    if ( ( !$subscription_method || $subscription_method == 'all' ) && ( $to_date >= $today ) ) {
      $editions = Paperlit_Editorial_Project::get_all_editions( $this->eproject );
    }
    elseif ( $subscription_method == 'last' ) {
      ...
      $editions = Paperlit_Editorial_Project::get_editions_in_range( $this->eproject, $today, $to_date );
    }

    if ( !empty( $editions ) ) {
      foreach ( $editions as $edition ) {
        $subscription_plans = get_post_meta( $edition->ID, '_paperlit_subscriptions_select', true );
        if ( !empty( $subscription_plans ) ) {
          if ( in_array( $subscription_bundle_id, $subscription_plans ) ) {
            $edition_bundle_id = Paperlit_Edition::get_bundle_id( $edition->ID, $this->eproject->term_id );
  ...
  public function validate_purchases_on_download( &$allow_download, $app_id, $device_id, $environment, $edition, $eproject ) {
    ...
    if ( !empty( $purchases['issues'] ) ) {
      // Get the editorial project settings
      $edition_bundle_id = Paperlit_Edition::get_bundle_id( $edition->ID, $eproject->term_id );
  ...
  protected function _action_purchase_confirmation() {
    ...
    $purchase_type = sanitize_text_field(esc_attr($_POST['type']));
    $product_id = isset( $_POST['product_id'] ) ? sanitize_text_field(esc_attr($_POST['product_id'])) : false;
    ...
    $this->base64_receipt = str_replace( ' ', '+', stripcslashes( sanitize_text_field(esc_attr($_POST['receipt_data'])) ) );
    ...
    if ( $receipt->transaction_id == $receipt->original_transaction_id ) {
      if ( $this->save_receipt( $receipt, $receipt->transaction_id, $purchase_type ) ) {
        Paperlit_Stats::increment_counter( 'purchase_' . $purchase_type );
  ...
  protected function _action_purchases_list() {
    ...
    $this->app_id = $wp->query_vars['app_id'];
    $this->device_id = $wp->query_vars['user_id'];
    $this->environment = isset( $_GET['environment'] ) ? sanitize_text_field(esc_attr($_GET['environment'])) : 'production';
...
$paperlit_server_connector_itunes = new Paperlit_Connector_iTunes;
