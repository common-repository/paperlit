<?php
define( 'PAPERLIT_THEME_CONFIG_FILE', 'config.xml' );

class Paperlit_Theme
{
	...
	public static function search_themes() {
		...
		if ( !$themes ) {
			...
			foreach ( $dirs as $dir ) {
				$files = Paperlit_Utils::search_files( $dir, 'xml', false );
				foreach ( $files as $file ) {
					$filename = basename( $file );
					if ( $filename != PAPERLIT_THEME_CONFIG_FILE ) {
						return;
					}

					array_push( $themes, array(
						'path' => str_replace( PAPERLIT_THEMES_PATH, '', dirname( $file ) ),
					));
				}
			}
	...
	public static function get_theme_settings( $edition_id ) {
		...
		$theme_id = get_post_meta( $edition_id, '_paperlit_theme_select', true );
		if ( $theme_id ) {
	...
	public static function get_theme_path( $edition_id, $absolute = true ) {
		...
		if ( $theme_id ) {
			...
			if( $absolute) {
				return PAPERLIT_THEMES_PATH . $options['path'] . DS;
			}
			else {
				return $options['path'];
			}
	...
	public static function get_theme_uri( $edition_id ) {
		...
		if ( $theme_id ) {
			$themes = self::get_themes();
			$options = $themes[$theme_id];
			return PAPERLIT_THEME_URI . $options['path'] . DS;
		}
	...
	public static function get_theme_layout( $edition_id, $rule ) {
		...
		if ( $theme_id ) {
			...
			foreach ( $options['layouts'] as $layout ) {
				if ( $layout['rule'] == $rule) {
					return PAPERLIT_THEMES_PATH . $options['path'] . DS . $layout['path'];
				}
			}
		}
	...
	public static function get_theme_assets_path( $edition_id, $absolute = true ) {
		...
		if ( $theme_id ) {
			...
			$options = $themes[$theme_id];
			if( $absolute) {
				return PAPERLIT_THEMES_PATH . $options['path'] . DS . $options['assets'] . DS;
			}
			else {
	...
	public static function get_theme_page( $edition_id, $post_id ) {
		...
		if ( $template ) {
			return PAPERLIT_THEMES_PATH . $template;
		}
	...
	public static function delete_theme( $theme_id ) {
		...
		if ( isset( $themes[$theme_id] ) ) {
			...
			$theme_path = PAPERLIT_THEMES_PATH . $theme_data['path'] . DS;
	...
	protected static function _scan_themes_dir() {
		...
		$themes = array();
		if ( is_dir( PAPERLIT_THEMES_PATH ) === false ) {
			return false;
		}

		try {
			$resource = opendir( PAPERLIT_THEMES_PATH );
			while ( false !== ( $file = readdir( $resource ) ) ) {

				if ( in_array( $file, Paperlit_Utils::$excluded_files) )	{
					continue;
				}

				$theme = PAPERLIT_THEMES_PATH . $file;
				if ( is_dir( $theme ) ) {
	...
	protected static function _validate_themes( $themes ) {
		...
		foreach ( $themes as $k => $theme ) {
			$config_file = PAPERLIT_THEMES_PATH . $theme['path'] . DS . PAPERLIT_THEME_CONFIG_FILE;
	...
	protected function _hooks() {
		add_action( 'admin_notices', array( 'PAPERLIT_Theme', 'themes_notice' ) );
	}
